 ...
 * Handles password protection, security questions, and attempt tracking for the payments section
 */

import { hashPassword } from './crypto'

// Storage keys
const PAYMENT_PASSWORD_KEY = 'dental_clinic_payment_password'
const SECURITY_QUESTION_KEY = 'dental_clinic_security_question'
const SECURITY_ANSWER_KEY = 'dental_clinic_security_answer'
const FAILED_ATTEMPTS_KEY = 'dental_clinic_payment_failed_attempts'
const LAST_ATTEMPT_TIME_KEY = 'dental_clinic_payment_last_attempt'
const PASSWORD_SETUP_COMPLETE_KEY = 'dental_clinic_payment_setup_complete'
...
export async function setupPassword(settings: PasswordSettings): Promise<boolean> {
  try {
    ...
    localStorage.setItem(PAYMENT_PASSWORD_KEY, hashedPassword)
...
export async function verifyPassword(password: string): Promise<boolean> {
  try {
    ...
    const storedHash = localStorage.getItem(PAYMENT_PASSWORD_KEY)
    if (!storedHash) {
      return false
    }
...
/**
 * Clear all payment security data (for testing/reset purposes)
 */
export function clearAllSecurityData(): void {
  try {
    localStorage.removeItem(PAYMENT_PASSWORD_KEY)
