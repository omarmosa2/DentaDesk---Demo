...
export default function PasswordSetupModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!securityQuestionId) {
      setError('يرجى اختيار سؤال أماني')
      return
    }
    ...
    if (securityAnswer.trim().length < 2) {
      setError('إجابة السؤال الأماني يجب أن تكون على الأقل حرفين')
      return
    }
    ...
    try {
      ...
      if (!selectedQuestion) {
        throw new Error('سؤال أماني غير صالح')
      }
      ...
      if (success) {
        toast({
          title: 'تم إعداد الأمان بنجاح',
          description: 'يمكنك الآن استخدام كلمة المرور لحماية قسم المدفوعات',
          variant: 'default',
        })
        onSuccess()
        onClose()
