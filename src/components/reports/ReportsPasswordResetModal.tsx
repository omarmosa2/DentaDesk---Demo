...
interface ReportsPasswordResetModalProps {
  isOpen: boolean
  onClose: () => void
  onSuccess: () => void
}

export default function ReportsPasswordResetModal({
  isOpen,
  onClose,
  onSuccess
}: ReportsPasswordResetModalProps) {
    ...
    try {
      ...
      if (success) {
        toast({
          ...
          description: 'يمكنك الآن استخدام كلمة المرور الجديدة للوصول إلى التقارير',
          variant: 'default',
        })
        onSuccess()
        onClose()
