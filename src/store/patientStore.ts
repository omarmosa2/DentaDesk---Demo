      ...
      loadPatients: async () => {
        ...
        try {
          ...
          if (isDemoMode) {
            ...
            patients = await mockDb.getAllPatients()
          } else {
            patients = await window.electronAPI?.patients?.getAll() || []
          }
      ...
      createPatient: async (patientData) => {
        ...
        try {
          ...
          if (isDemoMode) {
            ...
            const { MockDatabaseService } = await import('../services/mockDatabaseService')
            const mockDb = new MockDatabaseService()
            newPatient = await mockDb.createPatient(patientData)
          } else {
            newPatient = await window.electronAPI.patients.create(patientData)
          }
      ...
      updatePatient: async (id, patientData) => {
        ...
        try {
          ...
          if (isDemoMode) {
            ...
            const { MockDatabaseService } = await import('../services/mockDatabaseService')
            const mockDb = new MockDatabaseService()
            updatedPatient = await mockDb.updatePatient(id, patientData)
          } else {
            updatedPatient = await window.electronAPI.patients.update(id, patientData)
          }
      ...
      deletePatient: async (id) => {
        ...
        try {
          ...
          if (isDemoMode) {
            ...
            const { MockDatabaseService } = await import('../services/mockDatabaseService')
            const mockDb = new MockDatabaseService()
            success = await mockDb.deletePatient(id)
          } else {
            success = await window.electronAPI.patients.delete(id)
          }
        ...
        try {
          if (query.trim() === '') {
            ...
            if (isDemoMode) {
              ...
              const { MockDatabaseService } = await import('../services/mockDatabaseService')
              const mockDb = new MockDatabaseService()
              searchResults = await mockDb.searchPatients(query)
            } else {
              // Search using the database search function
              searchResults = await window.electronAPI.patients.search(query)
            }
