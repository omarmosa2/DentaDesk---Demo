...
ipcMain.handle('license:getStorageInfo', async () => {
  try {
    ...
    if (isDemoMode) {
      console.log('🎭 Demo Mode: Storage info bypassed')
      return {
        used: 0,
...
ipcMain.handle('license:getPredefinedLicenses', async (_, category = null) => {
  try {
    ...
    if (isDemoMode) {
      console.log('🎭 Demo Mode: Predefined licenses bypassed')
      return {
        licenses: [],
        category: category || 'demo',
        statistics: { total: 0, used: 0, available: 0 }
      }
    }
...
ipcMain.handle('license:searchPredefinedLicenses', async (_, searchTerm) => {
  try {
    ...
    if (isDemoMode) {
      console.log('🎭 Demo Mode: License search bypassed')
      return {
        results: [],
        searchTerm: searchTerm || 'demo'
      }
    }
...
ipcMain.handle('license:getRandomPredefinedLicense', async (_, category = null) => {
  try {
    ...
    if (isDemoMode) {
      console.log('🎭 Demo Mode: Random license bypassed')
      return {
        license: 'DEMO-LICENSE-KEY'
      }
    }
